...
*/
    ...
    public function editAction(Request $request, espece $espece)
    {
        ...
        if ($editForm->isSubmitted()) {
            ...
            $espece->setImage(''.$file->getClientOriginalName());
 ...
 *
        ...
        $saison = $this->getDoctrine()->getManager()
            ->getRepository('EspeceBundle:saison')
            ->findOneBy(array('nom'=>$request->get("saison")));
        ...
        $espece->setIdS($saison);
       $image = $_FILES['espImg']['name'];
        $target_path = "uploads/";
        $dir = sys_get_temp_dir();
        $uid = uniqid();
        $file = $uid .$image.".png" ;
        move_uploaded_file($_FILES["espImg"]["tmp_name"],$target_path . $file);
        $espece->setImage($file);
