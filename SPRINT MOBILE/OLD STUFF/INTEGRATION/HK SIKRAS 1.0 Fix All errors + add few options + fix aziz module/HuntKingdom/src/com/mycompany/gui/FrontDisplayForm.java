    ...
    public void Display(ArrayList<Produit> Produits) {
        ...
        for (Produit p : Produits) {
            ...
            fprod2.addPointerPressedListener(e -> {
                ...
                //----------------------------------Commande----------------------------------
                Button commande = new Button("Acheter");
                details.add(commande);
                SrProduct sr = new SrProduct();
                commande.addActionListener(ev -> {
                    if (sr.exist(p) == 0) {
                        sr.addToPanier(p);
                        listeprod.add(p);
                        Panier s = new Panier();
                        s.getF().showBack();
                        Dialog.show("INFORMATION", p.getNom() + " est ajouter", "ok", null);

                    } else {
                        Dialog.show("Warrning", "produit exist deja", "OK", null);
                    }
                });
                //----------------------------------Reclamation----------------------------------
                ...
                bdetails.addActionListener(ev -> {
                    ...
                    ToastBar.showErrorMessage("Produit ajoutee a votre wishlist");
        ...
        current.getToolbar().addCommandToLeftBar("Wishlist", null, (ev) -> {
            WishlistForm form;
            try {
                form = new WishlistForm();
                form.setScrollable(true);
