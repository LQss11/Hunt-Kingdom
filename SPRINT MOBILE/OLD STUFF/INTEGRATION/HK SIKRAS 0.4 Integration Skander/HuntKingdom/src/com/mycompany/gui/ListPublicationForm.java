...
public ArrayList<publication> publications = new ArrayList<>();
 ...
 setTitle("Liste publications");
     ...
     //current = new Form("search", BoxLayout.y());
        ...
        btsearch.addActionListener(e -> {
            ...
            publications = sp.getAllpublications();
            if (tfsearch.getText() != "") {
                publications = sp.getSearchListPublication(tfsearch.getText());
                current.revalidate();
                current.removeAll();
                current.add(fsearch);
                Display(publications);
 ...
 ServicePublication sp = new ServicePublication();
        for (publication P : sp.getInstance().getAllpublications()) {
            ...
            String url = Statics.ROJECT_UPLOADS_URL + "/" + P.getType();
            ...
            Button FindPublication = new Button("Voire Plus");
            ...
            btnRemovePublication.addActionListener((e) -> {
                try {
                    ...
                    if (ServicePublication.getInstance().RemovePublication(pub)) {
                        Dialog.show("Success", "publication avec ID= " + pub.getId() + " a ete supprimee avec succees", "OK", null);
...
FindPublication.addActionListener(e -> new FindPublicationForm(current, P).show());
        ...
        getToolbar().addMaterialCommandToLeftBar("Home", FontImage.MATERIAL_ARROW_DROP_UP, e -> new ForumIndex().show());
