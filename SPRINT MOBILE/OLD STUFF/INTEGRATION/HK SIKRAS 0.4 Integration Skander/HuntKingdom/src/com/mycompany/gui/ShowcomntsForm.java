    ...
    public ShowcomntsForm(Form previous) {
        ...
        if (comnts.isEmpty()) {
            ...
            for (commentaire P : comnts) {
                ...
                cnt1.add(lbDesc).add(lbDate).add(lbRate).add(lbSeparator).add(user);

                Container cnt2 = new Container(BoxLayout.x());
                Button AjouterCommentaire = new Button("Commenter");
                //Button FindPublication = new Button("Suprimer");
        ...
        getToolbar().addMaterialCommandToLeftBar(current.getTitle(), FontImage.MATERIAL_ARROW_BACK, e -> current.showBack());
    }
