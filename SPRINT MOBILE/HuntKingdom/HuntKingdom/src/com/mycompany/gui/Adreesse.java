...
public class Adreesse {
    ...
    void passecmd(UtilisateursAdresses ad) {
        ...
        c.setCommande(List);
        ArrayList<Commandes> cmds = ServiceCommandes.getInstance().getAllCommandes();
        if (cmds.size() > 0) {
            c.setId(cmds.get(cmds.size() - 1).getId() + 1);
        } else {
            c.setId(1);//error
        }
        ...
        try {
            ...
            Map<String, Object> link2 = j.parseJSON(new CharArrayReader(link.toCharArray()));

            System.out.println("Link2 = " + link2.get("link").toString());
            loadingAnimation.dispose();
            Paypal pay = new Paypal(theme, link2.get("link").toString());
            pay.getF().show();
        } catch (IOException ex) {
        }
