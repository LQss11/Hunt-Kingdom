...
class DefaultController extends Controller
{
    ...
    public function allinscritAction()
    {
        ...
        $serializer = new Serializer([new DateTimeNormalizer(), $normalizer]);
        //$formatted = $serializer->normalize($tasks);
        $formatted = $serializer->normalize($tasks,'json', [AbstractNormalizer::ATTRIBUTES => ['id','idevent'=>['id'],'iduser'=>['id'],'rate']]);
        return new JsonResponse($formatted);
    }
    ...
    public function inscritAction(Request $request)
    {
        ...
        $inscription->setRate($request->get('r'));
    ...
    public function rateAction(Request $request)
    {
        ...
        $dql = $this
            ...
            ->createQuery( 'SELECT I FROM  EvenementBundle:Inscription I WHERE I.id = :id');
        $dql->setParameter(':id', $request->get('id'));
        $alli= $dql->getResult();
        foreach ($alli as $i)
        {
            if ($i->getId()==$request->get('id')) {
                $inscription = new Inscription();

                $inscription->setIdevent($i->getIdevent());
                $inscription->setIduser($i->getIduser());
                $inscription->setRate($request->get('rate'));
                $em->persist($inscription);
                $em->flush();
