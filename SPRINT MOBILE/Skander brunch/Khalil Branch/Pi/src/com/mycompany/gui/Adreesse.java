...
public class Adreesse {
    ...
    public Adreesse(Resources theme) {
        ...
        Button btnsup = new Button(theme.getImage("rm.png"));
            btnsup.setUIID("label");
            btnsup.addActionListener((evt) -> {
                sr.RAdresses(ad.getId());
                Adreesse fad = new Adreesse(theme);
                fad.getF().showBack();
            });
            c.add(btnsup);
            f.add(c);
        ...
        f.getToolbar().addCommandToOverflowMenu("Ajouter adresse", null, (evt) -> {
            AddAdresse pn = new AddAdresse(theme);
            pn.getF().show();
        });
   ...
   void passecmd(UtilisateursAdresses ad) {
    ...
    /*    Double tot = 0.0;
        ...
        String idcmd = sr.addcmd(c,MyApplication.user);
