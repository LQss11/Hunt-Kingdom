...
public class ReclamationOwnController implements Initializable {
    ...
    @FXML
    private void afficherCell(ActionEvent event) throws IOException {
        buttonRetourMesReclamation.getScene().getWindow().hide();
        TableColumn.CellEditEvent edittedcell = null;
        Reclamation x = gettempReclamation(edittedcell);
        try {
            ...
            Reclamation.put("Description", x.getDescription().toString());
            Reclamation.put("Sujet", x.getSujet().toString());
            Reclamation.put("Type", x.getType().toString());
