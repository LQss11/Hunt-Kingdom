...
public class ReclamationOwnController implements Initializable {
    ...
    private void loadDataReclamation() {
        ...
        int info2 = UserId.getInt("UserId", 1);
            pst = con.prepareStatement("Select * from reclamation Where idU='" + info2 + "'");

            rs = pst.executeQuery();
            while (rs.next()) {
                dataReclamation.add(new Reclamation(rs.getInt("id"), rs.getString("type"), rs.getInt("ido"), rs.getString("sujet"), rs.getString("description"), rs.getDate("date"), rs.getString("etat")));
            }
